...
const Createalbum = () => {
  let [albumdetails, setAlbumDetails] = useState({
    title: "", lang: "", albumtype: "", description: "",
    daterelease: "", numberofsongs: "", starcast: "", director: "",
    songs: [], Thumbnail: "",
  });
  let { title, lang, albumtype, description, daterelease, numberofsongs, starcast, director, Thumbnail } = albumdetails;

  let [songdetails, setSongDetails] = useState([
    { songTitle: "", songSingers: "", songMusicDirectors: "", songThumbnail: "", songAudio: "" },
  ]);
  ...
  async function handlesubmit(e) {
    ...
    try {
      ...
      if (Thumbnail) {
        ...
        AlbumData.append("cloud_name", "dp3o3zo13");

        let AlbumResponse = await fetch(
          "https://api.cloudinary.com/v1_1/dp3o3zo13/image/upload",
          {
            method: "POST",
            body: AlbumData,
          }
      ...
      await Promise.all(
         songdetails.map(async (song, index) => {
          ...
          if (song.songThumbnail) {
            ...
            songThumbnailData.append("cloud_name", "dp3o3zo13");

            let songThumbnailResponse = await fetch(
              "https://api.cloudinary.com/v1_1/dp3o3zo13/upload",
              {
                method: "POST",
          ...
          if (song.songAudio) {
            ...
            songAudioData.append("cloud_name", "dp3o3zo13");
      ...
      <main>
        ...
        >
          ...
          <div className="pt-4 text-center">
            <button
              disabled={loading}
              className="w-full bg-purple-600 text-white py-2 px-4 rounded-md hover:bg-green-600 disabled:opacity-50 disabled:cursor-not-allowed transition"
            >
              {loading ? "Uploading... please wait ⏳" : "Create Album"}
            </button>
          </div>
        </form>
      </main>
